...
router.post('/', authenticate, requireRole('teacher', 'admin'), idempotency(), upload.single('file'), asyncHandler(textbookController.createTextbook));
...
router.post('/:textbookId/reprocess', authenticate, requireRole('teacher', 'admin'), idempotency(), validate(reprocessSchema), asyncHandler(textbookController.reprocessTextbook));
