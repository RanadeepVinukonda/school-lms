...
router.get('/config/:staffId', authenticate, requireRole('admin', 'super_admin'), asyncHandler(async (req: Request, res: Response) => {
  const result = await payrollService.getSalaryConfig(req.user!.school_id || '', req.params.staffId);
...
router.get('/runs/:id/payslip', authenticate, asyncHandler(async (req: Request, res: Response) => {
  const buffer = await payrollService.generatePayslipPdf(req.user!.school_id || '', req.params.id);
