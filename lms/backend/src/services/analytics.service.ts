...
export async function getConceptOversight() {
  ...
  // Resolve textbookId per assignment
  ...
  // Batch-load chapters, concepts, and all assessment docs
  ...
  const chapterByTextbook = new Map<string, string[]>(); // textbookId → chapterIds
  ...
  const allConceptIds = [...new Set((allConcepts || []).map((c: any) => c.id))];

  // Batch-load quiz & assignment docs by conceptId + classId
  const classConceptPairs = resolved.flatMap((a: any) => allConceptIds.map((cid: string) => `${a.classId}|${cid}`));
  ...
  // Build lookup: conceptId → [quizIds], conceptId → [assignmentIds]
...
export async function getConductedTests() {
  ...
  // Batch-load all attempt/submission data
  ...
  const attemptPercentages = new Map<string, number[]>(); // docId → percentages
