                ...
                <table className="w-full text-left min-w-[600px]">
                  <thead>
                    <tr className="border-b border-border/60 bg-muted/30 text-label-sm font-bold text-muted-foreground uppercase tracking-wider">
                      <th className="px-3 py-3.5 text-center w-14">Period</th>
                      {DAYS.map((day) => (
                        <th key={day} className="px-3 py-3.5 text-center min-w-[100px]">{DAY_SHORT[day] || day.slice(0, 3)}</th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {periodNumbers.map((period) => (
                      <tr key={period} className="hover:bg-muted/5 transition-colors">
                        ...
                        {DAYS.map((day) => {
                          ...
                          return (
                            <td key={`${day}-${period}`} className="px-2 py-3 align-top">
                              {entries.length === 0 ? (
                                <div className="min-h-[48px] flex items-center justify-center rounded-lg border border-dashed border-border/30 bg-muted/5">
