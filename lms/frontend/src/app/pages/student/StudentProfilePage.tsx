    ...
    <>
      ...
      >
        ...
        <DataFetchWrapper
          data={data}
          isLoading={isLoading}
          error={isError ? error ?? new Error('Failed to load profile') : null}
          loadingType="profile"
          emptyMessage="Could not load profile information"
          onRetry={() => refetch()}
            ...
            <>
              <motion.div variants={listItem} initial="hidden" animate="show">
                <Card variant="elevated">
                  <CardContent className="p-6 flex flex-col sm:flex-row items-center gap-5">
                    <div className="h-20 w-20 rounded-full bg-primary-container flex items-center justify-center flex-shrink-0">
                      <Icon name="account_circle" size={40} className="text-primary" />
                    </div>
                    <div className="flex-1 text-center sm:text-left">
                      <h2 className="text-xl font-bold">{d.student.displayName}</h2>
                      <p className="text-body-md text-muted-foreground">
                        Student &middot; ID: {d.student.studentId}
                      </p>
              ...
              <motion.div variants={listItem} initial="hidden" animate="show">
                <Card variant="elevated">
                  ...
                  <CardContent>
                    {d.enrolledSubjects.length === 0 ? (
                      <div className="flex flex-col items-center py-6 text-center">
                        <Icon name="menu_book" size={32} className="text-muted-foreground/50 mb-2" />
                        <p className="text-body-md text-muted-foreground">Not enrolled in any subjects</p>
                      </div>
                    ) : (
                      <motion.div
                  ...
                  <CardContent className="p-0">
                    {d.grades.length === 0 ? (
                      <div className="flex flex-col items-center py-8 text-center px-6">
                        ...
                        <p className="text-body-md text-muted-foreground">No grades recorded yet</p>
                      </div>
                    ) : (
                      <Table>
                        <TableHeader>
                          ...
                          {d.grades.map((grade) => {
                            ...
                            return (
                              <TableRow key={grade.id}>
                                ...
                                <TableCell className="text-center">
                                  <span className={cn('text-body-md font-bold', gradeColor)}>
                                    {letterGrade}
                                  </span>
                                </TableCell>
