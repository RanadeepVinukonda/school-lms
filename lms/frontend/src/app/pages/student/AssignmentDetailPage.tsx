...
import {
  ...
  Upload, Download, Calendar, Loader2, X
} from 'lucide-react';
...
export default function AssignmentDetailPage() {
  ...
  const assignment: {
    ...
    points: number; status: string; instructions: string;
    attachments: { id: string; name: string; size: string }[];
    submission: null; grade: { score: number; feedback: string } | null;
  } = {
    id: assignmentId!,
    ...
    points: 50,
    status: 'pending',
    instructions: '<p>Complete the following problems from Chapter 3:</p><ol><li>Solve for x: 2x + 5 = 13</li><li>Graph the equation y = 3x - 2</li><li>Find the slope between points (2,5) and (4,9)</li></ol><p>Show all your work for full credit.</p>',
    attachments: [{ id: 'a1', name: 'Chapter 3 Problems.pdf', size: '1.5 MB' }],
    submission: null,
    grade: null,
  };
    ...
    return (
      <div className="p-4">
        <Card><CardContent className="flex flex-col items-center gap-4 py-12">
          ...
          <p className="font-medium">Failed to load assignment</p>
      ...
      <Card className="mb-4">
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between">
            ...
            <Badge variant={isOverdue ? 'destructive' : assignment.status === 'submitted' ? 'success' : 'warning'}>
              {isOverdue ? 'Overdue' : assignment.status === 'submitted' ? 'Submitted' : 'Pending'}
            </Badge>
          </div>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          {assignment.attachments.map(a => (
            <div key={a.id} className="flex items-center justify-between p-2 rounded-lg bg-muted">
              <div className="flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /><span className="text-sm">{a.name}</span><span className="text-xs text-muted-foreground">({a.size})</span></div>
        ...
        <Card className="border-emerald-500/30">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Score: {assignment.grade.score}/{assignment.points} ({Math.round(assignment.grade.score / assignment.points * 100)}%)</p>
              {assignment.grade.feedback && <p className="text-sm mt-1">{assignment.grade.feedback}</p>}
            </div>
          </CardContent>
        </Card>
      ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <Card className="w-full max-w-sm">
            ...
            <CardContent className="flex gap-3">
              ...
              <Button className="flex-1" onClick={() => submitMutation.mutate()}>Confirm</Button>
            </CardContent>
          </Card>
        </div>
      )}
