...
/**
 * Reusable academic-year management UI (list + create/edit + set-as-current).
 * Fires `onYearChanged` after any mutation so the active-year context and every
 * portal (reports, attendance, fees...) can pick up the new current year.
 */
export function AcademicYearsManager({ onYearChanged }: { onYearChanged?: () => void }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Academic year created'); closeDialog(); invalidateAndSync(); },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Academic year updated'); closeDialog(); invalidateAndSync(); },
    onError: (err: Error) => toast.error(err.message || 'Failed to update'),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/academic-years/${id}`),
    onSuccess: () => { toast.success('Academic year deleted'); invalidateAndSync(); },
    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-body-md text-muted-foreground">Set the current academic year — every portal (reports, attendance, fees) follows it.</p>
        </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <p className="text-label-xs text-muted-foreground">
              The year marked <b>Current</b> is the single source of truth for report cards, attendance, fees and every portal — no hardcoding.
            </p>
          </div>
          <DialogFooter>
