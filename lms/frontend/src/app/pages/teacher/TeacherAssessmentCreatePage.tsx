...
export default function TeacherAssessmentCreatePage() {
  ...
  const updateReviewQuestion = useCallback((id: string, field: string, value: unknown) => {
    setReviewQuestions((prev: any[]) =>
      prev.map((q) => (q.id === id ? { ...q, [field]: value } : q))
    );
  }, []);

  const updateReviewOption = useCallback((questionId: string, optionIndex: number, value: string) => {
    setReviewQuestions((prev: any[]) =>
      prev.map((q) => {
        if (q.id !== questionId) return q;
        const options = [...(q.options || [])];
  ...
  const addReviewOption = useCallback((questionId: string) => {
    setReviewQuestions((prev: any[]) =>
      prev.map((q) =>
  ...
  const removeReviewOption = useCallback((questionId: string, optionIndex: number) => {
    setReviewQuestions((prev: any[]) =>
      prev.map((q) => {
        if (q.id !== questionId) return q;
  ...
  const updatePaperQuestion = useCallback((id: string, field: string, value: unknown) => {
    setAssignmentGeneratedPaper((prev: any[] | null) =>
      prev ? prev.map((q) => (q.id === id ? { ...q, [field]: value } : q)) : null
    );
  }, []);

  const updatePaperOption = useCallback((questionId: string, optionIndex: number, value: string) => {
    setAssignmentGeneratedPaper((prev: any[] | null) =>
      prev ? prev.map((q) => {
        if (q.id !== questionId) return q;
        const options = [...(q.options || [])];
  ...
  const addPaperOption = useCallback((questionId: string) => {
    setAssignmentGeneratedPaper((prev: any[] | null) =>
      prev ? prev.map((q) =>
        q.id === questionId ? { ...q, options: [...(q.options || []), ''] } : q
      ) : null
  ...
  const removePaperOption = useCallback((questionId: string, optionIndex: number) => {
    setAssignmentGeneratedPaper((prev: any[] | null) =>
      prev ? prev.map((q) => {
        if (q.id !== questionId) return q;
        const options = (q.options || []).filter((_: any, i: number) => i !== optionIndex);
                      ...
                      <div className="space-y-3 p-4 rounded-lg border border-border/60 bg-muted/20 mb-4">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {reviewQuestions.length > 0 && (
                            <Button size="sm" variant="outline" onClick={() => { setQuizGeneratedPaper(null); setReviewQuestions([]); }}>
                              Clear Preview
                            </Button>
                          )}
                        </div>
                        ...
                        {reviewQuestions.length > 0 && (
                          <div className="border rounded-lg p-3 bg-background space-y-3 max-h-80 overflow-y-auto">
                            <div className="flex items-center justify-between">
                              <p className="text-xs font-semibold text-primary">Preview ({reviewQuestions.length} questions)</p>
                              ...
                              <div key={q.id || i} className="rounded-lg border border-border/60 p-3 space-y-2">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <Input
                                    ...
                                    min={1}
                                    value={q.points ?? 1}
                                    onChange={(e) => updateReviewQuestion(q.id, 'points', Number(e.target.value))}
                                    className="w-20 h-7 text-xs"
                                  />
                                </div>
                                <textarea
                                  value={q.text || ''}
                                  onChange={(e) => updateReviewQuestion(q.id, 'text', e.target.value)}
                                ...
                                {['mcq', 'multiple_choice', 'fill_blank', 'matching'].includes(q.type) && q.options && (
                                  <div>
                                    <div className="flex items-center justify-between mb-1">
                                      ...
                                      <Button variant="ghost" size="icon-sm" onClick={() => addReviewOption(q.id)}>
                                      ...
                                      {q.options.map((opt: string, oi: number) => {
                                        ...
                                        <div key={oi} className="flex items-center gap-1.5">
                                          ...
                                          {q.options.length > 1 && (
                                            <button
                                              type="button"
                                              onClick={() => removeReviewOption(q.id, oi)}
                                ...
                                <div>
                                  ...
                                  {['mcq', 'multiple_choice'].includes(q.type) && q.options ? (
                                    <select
                                      ...
                                      onChange={(e) => updateReviewQuestion(q.id, 'correctAnswer', e.target.value)}
