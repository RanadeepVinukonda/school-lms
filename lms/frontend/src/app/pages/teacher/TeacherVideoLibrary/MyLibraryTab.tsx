    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>{allVideos.length} video{allVideos.length !== 1 ? 's' : ''}</span>
          <span className="text-muted-foreground/50">|</span>
          <span>{attachedCount} attached</span>
          <span className="text-muted-foreground/50">|</span>
          <span>{unattachedCount} unattached</span>
        </div>
        <div className="flex items-center gap-2 flex-wrap sm:ml-auto">
          <Select value={selectedTextbookId} onValueChange={(v) => { setSelectedTextbookId(v); setSelectedChapterId(''); setSelectedConceptId(''); }}>
            ...
            <SelectContent>
              <SelectItem value="">All textbooks</SelectItem>
              {textbooks.map((tb) => (
      ...
      <DataFetchWrapper
        ...
        emptyMessage={
          selectedTextbookId || selectedChapterId || selectedConceptId
            ...
            : 'Your video library is empty. Search YouTube to add videos.'
        }
        emptyAction={
          !selectedTextbookId && !selectedChapterId && !selectedConceptId ? (
      ...
      <ConfirmDialog
        ...
        title="Delete Video"
        description={`Remove "${deleteTarget?.title}" from your video library? This action cannot be undone.`}
