...
export const userService = {
  ...
  async getById(id: string) {
    const response = await api.get<ApiResponse<UserProfile>>(`/auth/users/${id}`);
    return response.data;
  },
  ...
  async toggleActive(id: string) {
    const response = await api.patch<ApiResponse<User>>(`/auth/users/${id}/toggle-active`);
    return response.data;
  },

  /** Update the current user's own profile. */
  async updateProfile(data: Partial<UserProfile>) {
    const response = await api.put<ApiResponse<UserProfile>>('/auth/users/profile', data);
    return response.data;
  },
