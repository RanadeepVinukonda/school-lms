...
The `users` table SHALL remove the single `role` column. A complete RBAC system SHALL be implemented with tables: `roles`, `permissions`, `role_permissions`, `user_roles`. Users SHALL hold multiple roles. Roles SHALL be school-scoped.

#### Scenario: User has multiple roles
- **WHEN** a user is assigned both teacher and parent roles
- **THEN** the system SHALL grant access to both teacher and parent features
- **THEN** the JWT SHALL include all active role identifiers

#### Scenario: RBAC enforces permission checks
- **WHEN** a user attempts an action without the required permission
- **THEN** the system SHALL return a 403 Forbidden response
- **THEN** the user SHALL not see the restricted UI elements

### Requirement: RLS policies on all business tables
PostgreSQL Row-Level Security SHALL be enabled on every business table. RLS policies SHALL enforce `school_id` isolation using a session-level setting.

#### Scenario: RLS prevents cross-school access
- **WHEN** a direct database query is executed without application filtering
- **THEN** RLS policies SHALL filter rows to the current school_id
- **THEN** cross-school data SHALL never be returned

## MODIFIED Requirements

### Requirement: Every entity is scoped to a school
Every database table representing school data SHALL have a `school_id` column referencing `schools(id)`. All queries SHALL include a `school_id` filter. RLS policies SHALL enforce school isolation at database level.
