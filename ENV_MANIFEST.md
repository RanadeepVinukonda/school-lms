...
## 14. Mobile APK (Capacitor) Vars

The Android APK is the Vite frontend in a Capacitor shell, so it uses the same `VITE_*` vars as the web build:

| Variable | Maps to backend var | Required | Default |
|---|---|---|---|
| `VITE_API_BASE_URL` | Backend API base URL | **Yes** | `http://localhost:3001/api` |
| `VITE_SUPABASE_URL` | `SUPABASE_URL` | **Yes** | — |
| `VITE_SUPABASE_ANON_KEY` | `SUPABASE_ANON_KEY` | **Yes** | — |

Set these in `lms/frontend/.env` before running `scripts/build-android-release.ps1`.
...
### Mobile APK `.env` (`lms/frontend/.env`, used by the Capacitor web build)

```bash
VITE_API_BASE_URL=http://localhost:3001/api                            # <--- CHANGE for production
VITE_SUPABASE_URL=https://<your-project>.supabase.co                   # <--- FILL
VITE_SUPABASE_ANON_KEY=eyJ...                                          # <--- FILL
```
